...
class RendezvousWatcher(threading.Thread):
  ...
  def run(self):
    """Main loop"""
    p = PicoRendezvous.PicoRendezvous()
    while 1:
...
class GrowlRelay(SocketServer.UDPServer):
  ...
  def __init__(self, password=None):
    ...
    UDPServer.__init__(self, ('', netgrowl.GROWL_UDP_PORT), _RequestHandler)
  ...
  def handle(self):
    ...
    if p.valid:
      ...
      for server in servers:
        s.sendto(p.data, (server, netgrowl.GROWL_UDP_PORT))
      s.close()
    else:
      servers = 'discarded'
