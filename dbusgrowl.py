...
class GrowlListener(SocketServer.UDPServer):
  ...
  def __init__(self, inpassword=None, outpassword=None):
    ...
    self.interface = dbus.Interface(notifyService,
                                    'org.freedesktop.Notifications')
    UDPServer.__init__(self,
                       ('localhost', netgrowl.GROWL_UDP_PORT),
                       _RequestHandler)
...
class _RequestHandler(SocketServer.DatagramRequestHandler):
  ...
  def handle(self):
    ...
    p = regrowl.GrowlPacket(self.rfile.read(),
                            self.server.inpassword,
                            self.server.outpassword)
    if p.type() == 'NOTIFY':
      notification, title, description, app = p.info()
      self.server.interface.Notify(
          app,
          0,
